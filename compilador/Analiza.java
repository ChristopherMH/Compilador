	...
	public void analizadorLexico(String token) 
	{
		...
		if(tipo==-1) {
			...
			if(mat.find())
				...
				resultado.add("Error léxico en la línea \""+renglon+"\" No. de token \""+columna+"\" nombre del token \""+token+"\", algunos signos no se admiten, los identificadores deben llevar al menos un número al final");
				tokenRC.add(new Token(token, renglon, columna, tipo));
				bandera = false;
				return;
			}
